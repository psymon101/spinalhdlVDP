/**
 * esp8266_copper_bars_demo.ino — Classic Amiga-style copper bars demo.
 *
 * Uses the mode2optimized FPGA's Copper to change BORDER_CTRL per scanline,
 * producing horizontal rainbow stripes across the full screen.
 *
 * Hardware: Tang Nano 20K (mode2optimized-gate2-enableL2L3 @ 22afb90)
...
/* Build copper program using WRITE (opcode 01) — 3 words per bar:
 *   WAIT(Y)
 *   WRITE(0x347)  → 0x4347
 *   data
 * Plus JUMP(0) at end.
...
static uint16_t build_copper_program(void)
{
    ...
    for (uint8_t bar = 0; bar < NUM_BARS; ++bar) {
        ...
        copper_prog[pc++] = (uint16_t)(0x4000u | (0x0347u & 0x3FFFu)); /* WRITE addr=0x347 */
...
void setup(void)
{
    ...
    Serial.println(F("Border: empty rect set."));
