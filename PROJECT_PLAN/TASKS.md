...
| **Summary** | Add a host-triggered soft reset that returns the VDP to a POR-equivalent state without a power cycle. Stage 1 (handshake + i80 `0x0310` live readback) and Stage 2 (on-chip host-writable Mem zero-sweep) are sim-proven. Stage 3 zeros only the **occupied/configured SDRAM regions** (auto-derived from active-layer geometry registers) via a controller-direct fill FSM in `sdramClockDomain`; full-chip refresh-interleave is no longer required because the sweep finishes under the 64 ms retention window. Stage 4 will partition the core ClockDomain so registers reset while the controller + host interface survive. `affineTexture` and immutable tile ROMs remain excluded from reset scope. |
| **Checkpoints** | A: `VDP_CTRL[2]` request/busy handshake + i80 live readback — DONE (`095a507`). B: On-chip Mem clear sweep — DONE (`a2043fc`). C: SDRAM occupied-region zero-fill engine — **IN PROGRESS** (design approved; implement + SDRAM-model cosim; no refresh interleave needed). D: Core register ClockDomain partition — **PENDING** (CyanPeak review before build). E: libvdp `vdp_mode0_soft_reset()` wrapper + docs — **PENDING** (1000 ms timeout remains). |
