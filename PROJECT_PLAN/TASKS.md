...
| **Status** | **DONE** — CyanPeak audit PASS #9278; BronzeGate PM closeout #9279 |
| **Phase** | closed |
| **Owner** | BrightForge (coding + proof), CyanPeak (audit PASS), CoralReef (ledger), BronzeGate (PM closeout) |
| **Baseline Commit** | `ab687d1` (Task 2a closeout) |
| **Commits** | `b2f4a5d` (CP-C evaluator RAM); `7b42b6a` (CP-D+E rasterizer narrow + VdpTop wiring); `b558cee` (CP-F final cleanup) |
| **Latest Auth Mail** | #9265–#9268 (BrightForge proof packet); #9276 (CoralReef ledger sync); #9278 (CyanPeak audit PASS); #9279 (BronzeGate closeout + Task 2b activation) |
| **Proof** | V=32: 13,940 logic (68%), 9,611 LUT, 7,726 FF, 0 unplaced REGs, 0 timing violations; V=8: 13,625 logic (66%), 9,103 LUT, 6,982 FF; regression 10/10 PASS |
| **Next Deliverable** | N/A — Task 2c closed |
...
**Current pivot:** The pixel-domain fetch path (narrow fix `527c026`, unified unblock `f4b04a9`) proved insufficient on hardware — uniform gray persisted despite sim PASS and synth clean. BronzeGate #9364 ruled Path A domain migration as the active unblock lane: move `PlanarLineFetch` / `BitplaneRowFetch` SDRAM consumption into `sdramClockDomain`, following the established `BitmapRowFetch` / `SdramTileAttributeFetch` pattern. BrightForge landed M1 sim-PASS at `cf5722e` (#9377) and M2 synth PASS #9386 (0 violations, 0 unplaced REGs). M3 hardware proof #9392 revealed planar bars ARE present end-to-end but layout is corrupted: hard boundary at x≈320, gray block x≈320..511, right-edge repeat x≈512..639. Paired diagnosis converged: CoralReef #9400 identified `VdpTop.scala:888` `hCounter.resize(9)` produces modulo-512 indexing instead of modulo-320, causing out-of-bounds `readAsync` on `planeMems` (slot 10..15, depth=10) → garbage/gray; BrightForge #9401 independently verified the mechanism against `cf5722e` HDL; CyanPeak #9403 concurs. PM #9402 authorized Option C: sim-only discriminator first, then one-line HDL fix `(hCounter % U(PLANE_PIXELS)).resize(9)`, then synth/flash/capture proof. Task 31 scroll-table dependency remains historical/closed context (`ac3fb87`) — not a separate active lane.
...
- #8817: **BronzeGate deep audit request to CoralReef.** Scope: review BrightForge work quality, probe chain correctness, capture discipline, next-step quality on Task 50. Requested independent verification of conclusions.
- #8816: **E3.15 result — SQUASH AT PALETTE.** BLUE (`bitmapFetch.io.pixelIndex != 0`) LIT, GREEN (`paletteRgb != 0`) DARK, RED (`io.red != 0`) DARK. Pipeline chain: `layer0Enable`→`fetchGrant`→`effectiveL0Enable`→`pixelIndex`→`drainWord` all LIT; `paletteRgb` first DARK. Diagnosis: Gowin BSRAM `readAsync` + `initialContent` defect affects palette Mem identically to LinestateStore. Proposed fix: replace palette Mem with Vec(Reg). Recommended 16 entries; CyanPeak authorized.
- #8815: **CyanPeak TASKS.md sync — #8814 integrated.** Updated ledger for E3.15 authorization.
- #8814: **CyanPeak audit ruling — E3.15 authorized.** Terminal-stage squash hunt: probe `io.red`, `paletteRgb`, `pixelIndex`. Capture via webcam fallback.
