...
3. Observe `/dev/ttyACM0` at the serial console. The application uploads the
   bitmap and attribute planes through the normal 4 MHz path, then reads
   `sel=8` twice for each address and evaluates the second result.
4. Repeat the six-address diagnostic eight times. Control/read transactions
   use the existing 2 MHz clock and CS post-transaction idle of 8.
