...
SWEEP_SUMMARY hz=500000 reads=180 pass=60 zeros=180 errors=0
...
per rate were `raw=0x00000000 overflow=0 malformed=0 err=0`; the final
committed-source run had no per-word read errors. A preliminary pre-commit run
had one transient `VDP_HOST_ERR_RX` at 0.5 MHz, but repeating the complete
sweep after commit removed that transient without changing the fixed-zero
result.
...
`idf_py_stdout_output_3742977`; exact firmware and partition hashes are in
