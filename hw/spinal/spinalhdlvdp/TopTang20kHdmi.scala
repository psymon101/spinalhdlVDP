...
// ============================================================================
// Lane #10590 Step 2 — scaler scenario tops (PM #10724).
// Each scenario top hardcodes SCALE_CTRL / LOGIC_WIDTH / LOGIC_HEIGHT /
// BORDER_CTRL POR inits to a different scale config so the bitstream comes
// up in that mode without firmware writes. Per AGENTS/BrightForge.md handoff
// rule, this avoids cross-agent firmware coordination for proof scaffolding.
// SCALE_CTRL bit layout: [7]=autoCenter, [6:4]=scaleY, [2:0]=scaleX.
// BORDER_CTRL bit layout: [12:8]=borderIdx, [0]=enable.
// ============================================================================

// Config B — 2x/2x no autoCenter. Used for the 3-build determinism panel.
// Scene is internally 2x-replicated but visually indistinguishable from the
// 1x baseline (default backdrop is palette[64]=black, scaling black gives
// black). Panel validates build determinism across the scaler-active path.
object TopTang20kHdmiScenarioScale2xVerilog extends App {
  Config.spinal.generateVerilog(TopTang20kHdmi(
    enableL1Fetch   = false,
    scaleCtrlInit   = 0x22,   // scaleX=2, scaleY=2, autoCenter=0
    logicWidthInit  = 320,
    logicHeightInit = 240,
  ))
}

// Config D — 3x/3x with autoCenter and visible white bezel (palette[1]).
// Expected: 80-pixel bezel on each horizontal side of the 640-wide active
// area ((640 - 3*160)/2 = 80). Bezel is BORDER_CTRL-driven, palette[1].
object TopTang20kHdmiScenarioScale3xACVerilog extends App {
  Config.spinal.generateVerilog(TopTang20kHdmi(
    enableL1Fetch   = false,
    scaleCtrlInit   = 0xB3,   // autoCenter=1, scaleY=3, scaleX=3
    logicWidthInit  = 160,
    logicHeightInit = 160,
    borderCtrlInit  = 0x0101, // borderIdx=1 (white), enable=1
  ))
}

// Config F — 4x/4x with autoCenter and visible white bezel (palette[1]).
// Expected: 64-pixel bezel on each horizontal side of the 640-wide active
// area ((640 - 4*128)/2 = 64). Bezel is BORDER_CTRL-driven, palette[1].
object TopTang20kHdmiScenarioScale4xACVerilog extends App {
  Config.spinal.generateVerilog(TopTang20kHdmi(
    enableL1Fetch   = false,
    scaleCtrlInit   = 0xC4,   // autoCenter=1, scaleY=4, scaleX=4
    logicWidthInit  = 128,
    logicHeightInit = 120,
    borderCtrlInit  = 0x0101, // borderIdx=1 (white), enable=1
  ))
