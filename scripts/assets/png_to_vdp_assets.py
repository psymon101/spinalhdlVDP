...
def emit_header(path: pathlib.Path, payload: dict) -> None:
    ensure_dir(path)
    guard = c_ident(path.stem)
    prefix = c_ident(path.stem)
    lines = [
        f"#ifndef {guard}",
        f"#define {guard}",
        "",
        "/* Auto-generated by scripts/assets/png_to_vdp_assets.py. */",
        f'/* Source: {payload["input"]} */',
        "",
        f'#define {prefix}_KIND "{payload["kind"]}"',
        f'#define {prefix}_INPUT "{payload["input"]}"',
    ]
    for key in ("tile_w", "tile_h", "sprite_w", "sprite_h", "bpp", "tiles_x", "tiles_y", "tiles", "sprites_x", "sprites_y", "sprites", "unique_tiles", "count"):
        if key in payload:
            value = payload[key]
            if isinstance(value, int):
                lines.append(f"#define {prefix}_{key.upper()} {value}u")
    outputs = payload.get("outputs", {})
    for name, value in outputs.items():
        lines.append(f'#define {prefix}_{name.upper()} "{value}"')
    if "output" in payload:
        lines.append(f'#define {prefix}_OUTPUT "{payload["output"]}"')
    if payload.get("sdram_base") is not None:
        lines.append(f"#define {prefix}_SDRAM_BASE 0x{int(payload['sdram_base']):X}u")
    lines += [
        "",
        f"#endif /* {guard} */",
        "",
...
def build_tilesheet(
    ...
    with open_image(input_path) as src:
        ...
        emit_manifest(output_prefix.with_suffix(".json"), payload)
        if header_path is not None:
            emit_header(header_path, payload)
...
def add_common_image_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        default=None,
        help="optional generated C header with asset metadata",
    )
    parser.add_argument(
        "--sdram-base",
        type=lambda s: int(s, 0),
        default=None,
        help="optional SDRAM byte base address to embed in the manifest/header",
    )
    ...
    if args.cmd == "background":
        ...
        with open_image(args.input) as src:
            ...
            payload = {
                ...
                "sdram_base": args.sdram_base,
            }
            write_palette(args.output, palette_from_image(img, args.count))
            emit_manifest(args.output.with_suffix(".json"), payload)
            if args.header is not None:
                emit_header(args.header, payload)
    else:
